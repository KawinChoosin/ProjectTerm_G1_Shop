...
const ProductList: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const sortedProducts = response.data.sort((a: Product, b: Product) => b.P_id - a.P_id);
    ...
    const applyFilter = () => {
        ...
        if (filter === 'low-stock') {
            ...
            filtered = filtered.filter((product) => product.CG_id === parseInt(filter));
        }
    ...
    const handleSave = async () => {
        ...
        formDataToSend.append('P_quantity', formData.P_quantity?.toString() as string);
        ...
        try {
            ...
            const response = await axios.put(`http://localhost:3000/products/${editingProduct.P_id}`, formDataToSend, {
                headers: {
            ...
            setFilteredProducts((prev) =>
                prev.map((product) => (product.P_id === editingProduct.P_id ? response.data : product))
            );
    ...
    const handleDelete = async (productId: number) => {
        if (window.confirm("Are you sure you want to delete this product?")) {
            try {
                ...
                setFilteredProducts((prev) => prev.filter((product) => product.P_id !== productId));
                    ...
                    <Box sx={{ marginBottom: 2 }}>
                        ...
                        {categories.map((cat) => (
                            <Button
                                ...
                                variant={filter === cat.CG_id.toString() ? 'contained' : 'outlined'}
                                sx={{ marginRight: 1 }}
                            >
                                {cat.CG_name}
                            </Button>
                        ...
                        <List>
                            {filteredProducts.map((product) => (
                                <ListItem key={product.P_id} divider>
                                    ...
                                    />
                                    <ListItemText
                                        primary={<Typography variant="h6">{product.P_name}</Typography>}
                                        secondary={
                                            <Box>
                                                ...
                                                <Typography variant="body2">
                                                    ...
                                                    {categories.find((cat) => cat.CG_id === product.CG_id)?.CG_name || 'Unknown'}
                                                </Typography>
                                            </Box>
                                        }
                                    />
