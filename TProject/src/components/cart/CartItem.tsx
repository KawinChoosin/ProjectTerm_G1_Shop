    ...
    <Box mb={2} p={2} display="flex" justifyContent="space-between" alignItems="center" borderBottom={1} borderColor="#e0e0e0" sx={{ borderColor: '#e0e0e0', borderBottomWidth: '1px' }}>
      <Box display="flex" alignItems="center" width="100%">
        <img
          ...
          style={{ width: '60px', height: '60px', objectFit: 'cover', marginRight: '16px', borderRadius: '4px' }}
        />
        <Box flex="1">
        ...
        <Box display="flex" alignItems="center">
          <IconButton onClick={handleDecrease} disabled={item.CA_quantity <= 1} size="small" sx={{ color: '#000' }}>
            <Remove />
          </IconButton>
          <Typography sx={{ margin: '0 10px' }}>{item.CA_quantity}</Typography>
          <IconButton onClick={handleIncrease} size="small" sx={{ color: '#000' }}>
            <Add />
          </IconButton>
        </Box>
        <Typography variant="h6" align="right" fontWeight="500" sx={{ width: '100px', textAlign: 'right' }}>
          ${(item.CA_quantity * item.P_price).toFixed(2)}
        </Typography>
        <IconButton onClick={handleDelete} size="small" sx={{ marginLeft: '16px', color: '#888' }}>
          <Close />
        </IconButton>
      </Box>
    </Box>
